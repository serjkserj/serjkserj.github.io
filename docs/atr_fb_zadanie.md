...
graph TD
    A[Начало: Инженеры создали/обновили кубики в SHAFT] --> B{Выбор варианта}
    B --> C[Полный цикл: Согласование ИОС → АР → КР в SHAFT]
    B --> D[Упрощенный: Обновление связи в AR]
    C --> E[Копирование согласованных кубиков в AR/KR]
    D --> F{Анализ изменений}
    F --> G[Незначительные: Ручная правка в AR]
    F --> H[Значительные: Удалить старые → Импортировать новые]
    E --> I[Создание отверстий: 'Соединение элементов']
    G --> I
    H --> I
